...
require(bool(summary_data.get("summary")), "摘要内容为空")
report["steps"].append({
    "step": "summarize",
...
require(cards_data.get("cards_count", 0) > 0, "知识卡内容为空")
report["steps"].append({
...
require(bool(notes_data.get("notes")), "结构化笔记内容为空")
require(
    {summary_source_id, cards_source_id, notes_source_id} == {summary_source_id},
    "相同文件没有复用同一资料源",
)
report["steps"].append({
    "step": "notes",
...
require(bool(single.json()["data"].get("content")), "摘要 artifact 内容为空")
report["steps"].append({
    "step": "get_artifact",
