...
_TOPICS = [
    ("操作系统", "进程与线程的区别是什么？", "进程是资源分配单位，线程是 CPU 调度单位；同进程内线程共享地址空间。"),
    ("计算机网络", "TCP 与 UDP 的主要区别是什么？", "TCP 面向连接、可靠、有序；UDP 无连接、不可靠、开销小。"),
    ("数据库", "ACID 分别指什么？", "原子性、一致性、隔离性、持久性。"),
    ("算法", "稳定排序和不稳定排序有什么区别？", "稳定排序保持相等元素的原始相对顺序。"),
    ("机器学习", "过拟合如何缓解？", "增加数据、正则化、早停、交叉验证。"),
    ("软件工程", "单元测试和集成测试的区别是什么？", "单元测试验证单一模块，集成测试验证模块间协作。"),
    ("数学", "导数的几何意义是什么？", "导数是函数在某点的切线斜率。"),
    ("编译原理", "词法分析和语法分析的区别是什么？", "词法分析生成 token，语法分析构建语法树。"),
    ("安全", "哈希函数和加密的区别是什么？", "哈希不可逆，加密可逆；哈希用于完整性，加密用于机密性。"),
